import Layout from '../components/Layout'
import Link from 'next/link'
import { img } from '../lib/img'

export default function Judo() {
  return (
    <Layout title="Judo" description="Judo-Training beim Kodokan Olsberg: Trainingszeiten, Gruppen und Informationen für Kinder, Jugendliche und Erwachsene.">

      <div className="page-header page-header--judo">
        <div className="container">
          <div>
            <div className="section-tag">Sportart</div>
            <h1>Judo</h1>
            <p>Der sanfte Weg – olympisch, traditionell, faszinierend</p>
          </div>
          <Link href="/aufnahmeantrag" className="btn btn-primary btn-sm">Jetzt anmelden</Link>
        </div>
      </div>

      <section className="section">
        <div className="container">
          <div className="sport-split">
            <div className="sport-img"><img src={img('/images/Judosport.png')} alt="Judo – Wurftechnik" /></div>
            <div>
              <div className="section-tag">Was ist Judo?</div>
              <h2 className="section-title">Die olympische Kampfkunst</h2>
              <p style={{color:'var(--text-muted)',lineHeight:'1.8',marginBottom:'1rem'}}>
                Judo – „der sanfte Weg" – wurde 1882 von Jigoro Kano in Japan entwickelt und ist seit 1964 olympische Disziplin.
                Im Wettkampf und im Training steht das Zusammenspiel von Technik, Taktik und mentaler Stärke im Vordergrund.
              </p>
              <p style={{color:'var(--text-muted)',lineHeight:'1.8'}}>
                Beim Kodokan Olsberg trainieren wir Judo für alle Altersgruppen – vom kleinen Bambini bis zum erfahrenen Wettkämpfer.
                Unsere Trainer begleiten jeden auf seinem individuellen Weg.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* TRAININGSZEITEN */}
      <section className="section section-alt">
        <div className="container">
          <div className="section-tag">Wann &amp; Wo</div>
          <h2 className="section-title">Trainingszeiten Judo</h2>
          <p className="section-lead">Trainingsstätte: <strong>Turnhalle der städtischen Real- und Sekundarschule Olsberg</strong>, Bahnhofstraße 59, 59939 Olsberg.</p>
          <div className="schedule-grid">

            {/* MONTAG */}
            <div className="day-card">
              <div className="day-label">Montag</div>
              <div className="session">
                <div className="session-time">⏱ 18:00 – 19:30</div>
                <div className="session-name">Kraft / Ausdauer</div>
                <div className="session-info">ab 7 Jahre</div>
                <div className="session-trainer">👤 Jamie</div>
              </div>
            </div>

            {/* DIENSTAG */}
            <div className="day-card">
              <div className="day-label">Dienstag</div>
              <div className="session">
                <div className="session-time">⏱ 18:00 – 19:15</div>
                <div className="session-name">Jugendtraining</div>
                <div className="session-info">6 – 11 Jahre</div>
                <div className="session-trainer">👤 Nico</div>
              </div>
              <div className="session">
                <div className="session-time">⏱ 19:30 – 21:00</div>
                <div className="session-name">Erwachsenentraining</div>
                <div className="session-info">ab 15 Jahre</div>
                <div className="session-trainer">👤 Nico</div>
              </div>
            </div>

            {/* DONNERSTAG */}
            <div className="day-card">
              <div className="day-label">Donnerstag</div>
              <div className="session">
                <div className="session-time">⏱ 16:30 – 17:45</div>
                <div className="session-name">Kindertraining</div>
                <div className="session-info">ab 4 Jahre</div>
                <div className="session-trainer">👤 Tobi / Tia / Albert / Jule</div>
              </div>
              <div className="session">
                <div className="session-time">⏱ 17:45 – 19:15</div>
                <div className="session-name">Leistungsgruppe</div>
                <div className="session-info">ab Orange Gurt</div>
                <div className="session-trainer">👤 Tobi / Tia</div>
              </div>
              <div className="session">
                <div className="session-time">⏱ 19:30 – 21:00</div>
                <div className="session-name">Erwachsenentraining</div>
                <div className="session-info">ab 15 Jahre</div>
                <div className="session-trainer">👤 Tobi</div>
              </div>
            </div>

            {/* FREITAG */}
            <div className="day-card">
              <div className="day-label">Freitag</div>
              <div className="session">
                <div className="session-time">⏱ 18:00 – 19:30</div>
                <div className="session-name">Wettkampftraining</div>
                <div className="session-info">ab Gelb Gurt</div>
                <div className="session-trainer">👤 Tia / Tobi</div>
              </div>
              <div className="session">
                <div className="session-time">⏱ 19:30 – 21:00</div>
                <div className="session-name">Kata / Spiel &amp; Spaß</div>
                <div className="session-info">Wochenwechsel</div>
                <div className="session-trainer">👤 Tia / Tobi</div>
              </div>
            </div>

          </div>
        </div>
      </section>

      {/* WARUM JUDO */}
      <section className="section">
        <div className="container">
          <div className="section-tag">Vorteile</div>
          <h2 className="section-title">Warum Judo?</h2>
          <div className="grid-3">
            <div className="card">
              <div className="card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z"/></svg></div>
              <h3>Körper &amp; Fitness</h3>
              <p>Judo trainiert die gesamte Muskulatur, verbessert Koordination, Ausdauer und Beweglichkeit.</p>
            </div>
            <div className="card">
              <div className="card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18"/></svg></div>
              <h3>Mentale Stärke</h3>
              <p>Fokus, Disziplin und Respekt – Werte, die auf der Matte erlernt und im Alltag gelebt werden.</p>
            </div>
            <div className="card">
              <div className="card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.955 11.955 0 013 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z"/></svg></div>
              <h3>Selbstverteidigung</h3>
              <p>Judotechniken sind effektive Selbstverteidigungsmethoden, die im Ernstfall wirklich funktionieren.</p>
            </div>
            <div className="card">
              <div className="card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/></svg></div>
              <h3>Ideal für Kinder</h3>
              <p>Judo fördert Teamgeist, Selbstbewusstsein und motorische Entwicklung – ideal ab dem Vorschulalter.</p>
            </div>
            <div className="card">
              <div className="card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 01-.982-3.172M9.497 14.25a7.454 7.454 0 00.981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 007.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M7.73 9.728a6.726 6.726 0 002.748 1.35m8.272-6.842V4.5c0 2.108-.966 3.99-2.48 5.228m2.48-5.492a46.32 46.32 0 012.916.52 6.003 6.003 0 01-5.395 4.972m0 0a6.726 6.726 0 01-2.749 1.35m0 0a6.772 6.772 0 01-3.044 0"/></svg></div>
              <h3>Wettkampfsport</h3>
              <p>Von Kreismeisterschaften bis zur Bundesliga – wer Wettkämpfe liebt, findet bei uns die richtige Plattform.</p>
            </div>
            <div className="card">
              <div className="card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253M3.284 14.253A8.959 8.959 0 013 12c0-.778.099-1.533.284-2.253"/></svg></div>
              <h3>Weltweit zuhause</h3>
              <p>Judo wird in 200 Ländern praktiziert – einmal Judoka, überall willkommen.</p>
            </div>
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="section">
        <div className="container text-center">
          <h2 className="section-title">Probetraining – kostenlos &amp; unverbindlich</h2>
          <p className="section-lead" style={{margin:'0 auto .75rem'}}>Qualität, Teamgeist und echte Förderung – bei uns findest du die besten Voraussetzungen, um sportlich und persönlich zu wachsen.</p>
          <p className="section-lead" style={{margin:'0 auto 2rem',fontSize:'.95rem'}}>Komm einfach vorbei! Sportkleidung reicht für den Anfang – ein Judogi kann später angeschafft werden.</p>
          <div style={{display:'flex',gap:'1rem',justifyContent:'center',flexWrap:'wrap'}}>
            <Link href="/aufnahmeantrag" className="btn btn-primary">Mitglied werden</Link>
            <Link href="/kontakt" className="btn btn-ghost">Fragen? Kontakt aufnehmen</Link>
          </div>
        </div>
      </section>

    </Layout>
  )
}
